...
##绘制有向无环图###
ego_MF <- enrichGO(gene = test1$ENTREZID,OrgDb = org.Hs.eg.db,ont = "MF", pAdjustMethod = "BH",pvalueCutoff = 0.05,qvalueCutoff = 0.05,readable = FALSE)
plotGOgraph(ego_MF)
ego_BP <- enrichGO(gene = test1$ENTREZID,OrgDb = org.Hs.eg.db,ont = "BP", pAdjustMethod = "BH",pvalueCutoff = 0.05,qvalueCutoff = 0.05,readable = FALSE)
plotGOgraph(ego_BP)
ego_CC <- enrichGO(gene = test1$ENTREZID,OrgDb = org.Hs.eg.db,ont = "CC", pAdjustMethod = "BH",pvalueCutoff = 0.05,qvalueCutoff = 0.05,readable = FALSE)
plotGOgraph(ego_CC)
##GO terms之间的基因重叠关系
